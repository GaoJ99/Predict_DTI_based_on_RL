1.the datasets folder contains the four benchmark datasets required

2.blm.py, cmf.py, nrlmf.py, wnngip.py and netlaprls.py, these five. py files are the codes for each of the five models

3.calculate_similarity_matrix.py includes a method for calculating the similarity between drugs and targets

4.rl_Qtable.py is the code for establishing the Q table

5.run_dtp2.py is the main code for building the QLDTI method
