...
class NRLMF:
    ...
    def __init__(self, cfix=5, K1=5, K2=5, num_factors=10, theta=1.0, lambda_d=0.625, lambda_t=0.625, alpha=0.1, beta=0.1, max_iter=100):
        self.cfix = int(cfix)  # importance level for positive observations  积极观察的重要性水平
        self.K1 = int(K1)
        self.K2 = int(K2)
    ...
    def fix_model(self, W, intMat, drugMat, targetMat, seed=None):
        ...
        x, y = np.where(self.intMat > 0)  # x, y均是一个数组，分别存放满足条件的坐标下标
        self.train_drugs, self.train_targets = set(x.tolist()), set(y.tolist())  # 将数组或矩阵转化为列表
